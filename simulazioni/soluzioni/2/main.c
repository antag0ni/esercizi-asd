...
void HT_rimuovi_libri_restituiti(THashTable* ht, TStack* stack)
{
    ...
    while (!stack_is_empty(stack))
        hashtable_delete(ht, stack_pop(stack).key);
}

TList evadi_prestiti(const TBinaryTree bst, THashTable* ht, TQueue* queue)
{
    /* COMPLETARE QUESTA FUNZIONE - SEGUE CODICE DA RIMUOVERE */
    TList new_list = list_create();
    TQueue new_queue = queue_create(10);
    TInfo libro;
    while (!queue_is_empty(queue))
    {
        libro = queue_remove(queue);
        if (binarytree_search(bst, libro)) {
            if (hashtable_search(ht, libro.key) == NULL)
            {
                new_list = list_insert(new_list, libro);
                hashtable_insert(ht, libro.key, libro.value);
            }
            else
                queue_add(&new_queue,libro);
        }
        else
            queue_add(&new_queue,libro);
    }
...
TList list_delete_odd(TList list)
{
    ...
    if (list == NULL)
        return list;
    list->link = list_delete_odd(list->link);
    if (!info_isEven(list->info))
    {
...
{
    /* COMPLETARE QUESTA FUNZIONE - SEGUE CODICE DA RIMUOVERE */
    // Se il nodo è NULL, altezza = 0 e considerato albero pieno
    if (root == NULL)
    {
        *height = 0;
        return 1;
    ...
    // Se il nodo è una foglia, è considerato un albero pieno
    ...
    int leftFull = is_full_binary_tree(root->left, &leftHeight);
    int rightFull = is_full_binary_tree(root->right, &rightHeight);

    // Calcolo dell'altezza attuale
    *height = (leftHeight > rightHeight ? leftHeight : rightHeight) + 1;

    // Se entrambi i sottoalberi sono presenti e sono pieni, restituisce vero
    return (root->left != NULL && root->right != NULL) && leftFull && rightFull;
}